...
int main()
{
  ...
  std::vector<std::string> plugins_list = nav2_util::split(project11_navigation::details::BT_BUILTIN_PLUGINS, ';');

  for (const auto & plugin : plugins_list) {
    std::cout << "Loading: " << plugin << "\n";
    factory.registerFromPlugin(BT::SharedLibrary::getOSName(plugin));
  }
  std::cout << "\nGenerating file: project11_navigation_tree_nodes.xml\n"
            << "\nCompare it with the one in the git repo and update the latter if necessary.\n";

  std::ofstream xml_file;
  xml_file.open("project11_navigation_tree_nodes.xml");
